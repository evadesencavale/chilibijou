...
import { ORDERED_QUESTION_TYPES } from '../constants/syllogimous.constants';
import {
  Question,
  IArrangementPremise,
  IDirectionProposition,
  IDirection3DProposition,
...
import {
  ...
  getRandomSymbols,
  getSyllogism,
  isPremiseLikeConclusion,
  shuffle,
  getRelation,
  createMetaRelationships,
  getLinearWays,
  getCircularWays,
  getSymbols,
  horizontalShuffleArrangement,
  metarelateArrangement,
...
export class SyllogimousService {
  ...
  getCreateFn(questionType: EnumQuestionType, numOfPremises: number) {
    return {
      [EnumQuestionType.Distinction]: () =>
        this.createDistinction(numOfPremises),
      ...
      [EnumQuestionType.GraphMatching]: () =>
        this.createGraphMatching(numOfPremises),
  ...
  createRandomQuestion(numOfPremises?: number, basic?: boolean) {
    ...
    const typeSettingTuples = Object.entries(settings.question) as [
      EnumQuestionType,
      QuestionSettings
    ][];
    const getQuestionGroup = (qg?: EnumQuestionGroup) =>
      typeSettingTuples.filter(([qt, qs]) => qs.group == qg);
    ...
    for (const grouped of groupsOfQuestions) {
      ...
      const groupChoices: Array<() => Question> = isUndefinedGroup
        ? choices
        : [];
      for (const [qt, qs] of grouped) {
        const shouldIncludeQuestion =
          basic == undefined ? true : qs.basic === basic;
        if (qs.enabled && shouldIncludeQuestion) {
          groupChoices.push(
            this.getCreateFn(
              qt,
              qs.clampNumOfPremises(numOfPremises || qs.getNumOfPremises())
            )
          );
        }
      }
  ...
  createSyllogism(numOfPremises: number) {
    ...
    do {
      question.rule = question.isValid
        ? getRandomRuleValid()
        : getRandomRuleInvalid();
      question.bucket = getRandomSymbols(settings, length);
      question.premises = [];

      [question.premises[0], question.premises[1], question.conclusion] =
        getSyllogism(
          settings,
          question.bucket[0],
          question.bucket[1],
    ...
    for (let i = 3; i < length; i++) {
      ...
      const flip = coinFlip();
      const [p, m] = flip
        ? [question.bucket[i], question.bucket[rnd]]
        : [question.bucket[rnd], question.bucket[i]];
      question.premises.push(
        getSyllogism(settings, '#####', p, m, getRandomRuleInvalid())[0]
      );
    }
  ...
  createDistinction(numOfPremises: number): Question {
    ...
    do {
      ...
      for (let i = 0; i < length - 1; i++) {
        ...
        question.premises.push(
          `<span class="subject">${prev}</span> is ${relation} <span class="subject">${curr}</span>`
        );
  ...
  createComparison(
    ...
    do {
      ...
      for (let i = 0; i < length - 1; i++) {
        ...
        const [first, last] =
          (sign === 1) === isMoreOrAfter ? [next, curr] : [curr, next];
        const relation = getRelation(settings, type, isMoreOrAfter);

        question.premises.push(
          `<span class="subject">${first}</span> is ${relation} <span class="subject">${last}</span>`
        );
      }
  ...
  createArrangement(
    ...
    question.instructions.push(
      `There are <b>${
        NUMBER_WORDS[numOfEls] || numOfEls
      } subjects</b> along a <b>${isLinear ? 'linear' : 'circular'}</b> path.`
    );

    const relationshipAlreadyExistent = (a: string, b: string) =>
      premises.find(
        ({ a: pA, b: pB }) => (pA === a && pB === b) || (pA === b && pB === a)
      );
    ...
    while (safe-- && premises.length < numOfEls - 1) {
      ...
      while (safe-- && premise == undefined) {
        ...
        // Pick B
        const b = pickUniqueItems(
          subjects.filter((sub) => sub !== a),
          1
        ).picked[0];
        ...
        const [wayDescription, wayData] = pickUniqueItems(
          Object.entries(getWays(aid, bid, numOfEls)),
          1
        ).picked[0];
    ...
    question.isValid = coinFlip();
    const conclusions = Object.entries(ways).filter(
      ([description, data]) => data.possible === question.isValid
    );
    ...
    const interpolated = interpolateArrangementRelationship(
      { description, steps },
      settings
    );
    ...
    question.premises = premises.map(
      ({ a, b, relationship, metaRelationships, uid }) => {
        if (
          settings.enabled.meta &&
          coinFlip() &&
          metaRelationships.length &&
          !metaRelationshipLookupMap[uid]
        ) {
  ...
  createDirection(numOfPremises: number): Question {
    ...
    while (
      safe-- &&
      (!coorda || !coordb || pairAlreadyEstablished(coorda[0], coordb[0]))
    ) {
    ...
    const getRelationship = (
      ...
      if (!tweaked && cardinals.every((c) => c[1] === 1)) {
        ...
        const numStepsVertical =
          NUMBER_WORDS[cardinals[0][1]] || cardinals[0][1];
        relationship =
          numStepsVertical +
          ' step' +
          (cardinals[0][1] > 1 ? 's' : '') +
        ...
        if (cardinals.length === 2) {
          const numStepsHorizontal =
            NUMBER_WORDS[cardinals[1][1]] || cardinals[1][1];
          relationship +=
            ' and ' +
            numStepsHorizontal +
            ' step' +
    ...
    if (isValid) {
      ...
      if (coinFlip() && conclusion.cardinals.length === 2) {
        ...
        conclusion.cardinals = [
          pickUniqueItems(conclusion.cardinals, 1).picked[0],
        ];
        tweaked = true;
      }
    } else {
      ...
      if (coinFlip()) {
        ...
        conclusion.cardinals[rndIdx][0] =
          cardinalOppositeMap[conclusion.cardinals[rndIdx][0]];
      }
      tweaked = true;
    }
    ...
    const stringifyProposition = (p: IDirectionProposition) => {
      const relationship = settings.enabled.negation
        ? negateRelationship(p.relationship)
        : p.relationship;
  ...
  createDirection3D(
    ...
    while (
      safe-- &&
      (!coorda || !coordb || subjectsAlreadyIncluded(coorda[0], coordb[0]))
    ) {
      [coorda, coordb] = pickUniqueItems(usedCoords, 2).picked;
    }
    ...
    for (const pair of pairs) {
      ...
      const trasversalRelationship =
        getTrasversalRelationship(trasversalDifference);
      const cardinalRelationship = getCardinalRelationship(cardinals);
      const connector =
        cardinalRelationship === SAME_CARDINAL_DIRECTION
          ? ' and '
          : cardinalRelationship.indexOf(' and ') > -1
          ? ', '
          : ' and ';
      const relationship =
        trasversalRelationship + connector + cardinalRelationship;
    ...
    if (isValid) {
      ...
      if (coinFlip() && conclusion.cardinals.length === 2) {
        this.logger.info(
          'Cardinal pluck before',
          JSON.stringify(conclusion.cardinals, null, 2)
        );
        conclusion.cardinals = [
          pickUniqueItems(conclusion.cardinals, 1).picked[0],
        ];
        this.logger.info(
          'Cardinal pluck after',
          JSON.stringify(conclusion.cardinals, null, 2)
        );
      }
    } else {
      ...
      conclusion.cardinals = conclusion.cardinals.filter(
        (c) => c[0] !== '!' && c[1] !== 0
      );
    ...
    // Regenerate conclusion relationship
    conclusion.trasversalDifference =
      conclusion.pair[0][3] - conclusion.pair[1][3];
    const trasversalRelationship = getTrasversalRelationship(
      conclusion.trasversalDifference
    );
  ...
  createGraphMatching(numOfPremises: number): Question {
    ...
    const _words = [...words];
    const isWordUsed = (w: string) =>
      edgeList
        .reduce(
          (a, c) => (a.add(c[0]), a.add(c[2]), a),
          new Set() as Set<string>
        )
        .has(w);
    const notAllUsed = () => _words.some((w) => !isWordUsed(w));
    const edgeAlreadyExists = (a: string, b: string) =>
      edgeList.some(
        ([_a, _, _b]) => (_a === a && _b === b) || (_a === b && _b === a)
      );
    let safe = 1e3;
    ...
    const all3ElementsAre2Way =
      numOfEls === 3 && edgeList.every(([a, rel, b]) => rel === '↔');
    ...
    if (!question.isValid) {
      ...
      while (areGraphsIsomorphic(edgeList, edgeList2)) {
        ...
        if (coinFlip() && numOfEls > 3) {
          ...
          while (
            !picked ||
            picked === picked2[0][subjectPosIdx] ||
            picked === picked2[0][subjectNegIdx]
          ) {
            picked = pickUniqueItems(newWords, 1).picked[0];
          }
          this.logger.info(
            'Change an edge by connecting a/b to a different subject',
    ...
    const readable = (
      ...
      if (meta) {
        ...
        if (
          ...
          if (coinFlip() && edge[1] !== '↔') {
            relationship = `the inverse of ${getSubject(
              pickedEdge[2]
            )} to ${getSubject(pickedEdge[0])}`;
          } else {
            relationship = `${getSubject(pickedEdge[0])} is to ${getSubject(
              pickedEdge[2]
            )}`;
          }
          isMetaRelated = true;
        ...
        question.negations++;
        relationship = `<span class="is-negated">${
          readMap[inverseMap[edge[1]]]
        }</span>`;
      }
      return isMetaRelated
        ? `${getSubject(edge[0])} is to ${getSubject(
            edge[2]
          )} as ${relationship}`
  ...
  createAnalogy(length: number) {
    ...
    switch (choiceIndex) {
      case 0:
        ...
        [a, b, c, d] = pickUniqueItems(
          [...question.buckets[0], ...question.buckets[1]],
          4
        ).picked;
      ...
      case 2:
        ...
        [indexOfA, indexOfB] = [
          question.bucket.indexOf(a),
          question.bucket.indexOf(b),
        ];
        [indexOfC, indexOfD] = [
          question.bucket.indexOf(c),
          question.bucket.indexOf(d),
        ];
        isValidSame =
          (indexOfA > indexOfB && indexOfC > indexOfD) ||
          (indexOfA < indexOfB && indexOfC < indexOfD);
      ...
      case 3:
        while (flip !== isValidSame) {
          ...
          const [coordsa, coordsb, coordsc, coordsd] = pickUniqueItems(
            question.coords,
            4
          ).picked;
      ...
      case 5: {
        ...
        while (flip !== isValidSame) {
          ...
          isValidSame =
            dxatob === dxctod && dyatob === dyctod && dtatob === dtctod;
        }
        break;
      }
      case 6:
      case 7: {
        ...
        if (isLinear) {
          ...
          question.notes.push(
            'Proximity and diametrical opposition makes the relationship alike.'
          );
        }
        ...
        for (const key in waysA2B) {
          if (
            waysA2B[key].possible &&
            waysC2D[key].possible &&
            waysA2B[key].steps === waysC2D[key].steps
          ) {
            isValidSame = true;
          }
        }
    ...
    if (settings.enabled.negation && coinFlip()) {
      question.negations++;
      question.conclusion += `<div class="analogy-conclusion is-negated">is ${
        isSameRelationship ? 'unlike' : 'alike'
      }</div>`;
    } else {
      question.conclusion += `<div class="analogy-conclusion">is ${
        isSameRelationship ? 'alike' : 'unlike'
      }</div>`;
    }
  ...
  createBinary(numOfPremises: number) {
    ...
    do {
      ...
      question.instructions = [
        fixBinaryInstructions(a),
        fixBinaryInstructions(b),
      ].filter((instr) => !!instr);
      ...
      question.conclusion = operandTemplates[operandIndex]
        .replace(
          '$a',
          Array.isArray(choices[0].conclusion)
            ? choices[0].conclusion[0]
            : choices[0].conclusion
        )
        .replace(
          '$b',
          Array.isArray(choices[1].conclusion)
            ? choices[1].conclusion[0]
            : choices[1].conclusion
        );
