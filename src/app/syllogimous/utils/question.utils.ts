...
export function extractSubjects(phrase: string) {
  return [...phrase.matchAll(/<span class="subject">(.*?)<\/span>/g)].map(
    (a) => a[1]
  );
}

export function isPremiseLikeConclusion(
  premises: string[],
  conclusion: string
) {
  ...
  for (const subjects of subjectsOfPremises) {
    ...
    if (
      subjects[0] + subjects[1] === toCompare ||
      subjects[1] + subjects[0] === toCompare
    )
      return true;
  }
  return false;
...
export function getMetaReplacer(
  settings: Settings,
  ...
  const choosenSubjects = [
    ...choosenPair.picked[0].matchAll(/<span class="subject">(.*?)<\/span>/g),
  ];
  const [a, b] = choosenSubjects.map((m) => m[1]);

  const isSameAs =
    (relations[0] === relations[1]) === (negations[0] === negations[1]);
...
export function createMetaRelationships(
  ...
  if (settings.enabled.meta && coinFlip()) {
    const numOfMetaRelationships =
      1 + Math.floor(Math.random() * Math.floor((length - 1) / 2));
    ...
    if (question.type === EnumQuestionType.Distinction) {
      subjects = question.buckets.reduce(
        (a, c, i) => [...a, ...c.map((b) => ({ value: i, subject: b[0] }))],
        [] as typeof subjects
      );
    } else {
      subjects = question.bucket.map(
        (c, i, a) => ({ value: a.length - i, subject: c }),
        []
      );
    }

    const { picked: pickedPremises, remaining: remainingPremises } =
    ...
    const bidirectionalRelationshipMap = remainingPremisesSubjects.reduce(
      (acc, [a, b]) => (
        (acc[a] = acc[a] || []),
        acc[a].push(b),
        (acc[b] = acc[b] || []),
    ...
    for (const premiseSubjects of pickedPremisesSubjects) {
      const [a, b] = premiseSubjects.map(
        (ps) => subjects.find((s) => ps === s.subject)!
      );
      const { picked } = pickUniqueItems(
        Object.entries(bidirectionalRelationshipMap),
        1
      );
      let _c = '';
      let _d = '';
      ...
      if (isSame) {
        ...
        if (settings.enabled.negation && coinFlip()) {
          newPremises.push(
            `<span class="subject">${a.subject}</span> relates to <span class="subject">${b.subject}</span> in the <span class="is-negated">opposite</span> way that <span class="subject">${c.subject}</span> relates to <span class="subject">${d.subject}</span>`
          );
        } else {
          newPremises.push(
            `<span class="subject">${a.subject}</span> relates to <span class="subject">${b.subject}</span> in the same way that <span class="subject">${c.subject}</span> relates to <span class="subject">${d.subject}</span>`
          );
        }
      } else {
        // Different
        if (settings.enabled.negation && coinFlip()) {
          newPremises.push(
            `<span class="subject">${a.subject}</span> relates to <span class="subject">${b.subject}</span> in the <span class="is-negated">same</span> way that <span class="subject">${c.subject}</span> relates to <span class="subject">${d.subject}</span>`
          );
        } else {
          newPremises.push(
            `<span class="subject">${a.subject}</span> relates to <span class="subject">${b.subject}</span> in the opposite way that <span class="subject">${c.subject}</span> relates to <span class="subject">${d.subject}</span>`
          );
        }
      }
    }
...
export function metarelateArrangement(premises: IArrangementPremise[]) {
  premises.forEach((premise) => {
    premise.metaRelationships = premises
      ...
      .filter(
        (p) =>
          p.relationship.description === premise.relationship.description &&
          p.relationship.steps === premise.relationship.steps
      );
  });
}
...
export function interpolateArrangementRelationship(
  ...
  const interpolatedWithSteps = relationship.description.replace(
    /# steps/,
    () =>
  ...
  if (settings.enabled.negation && coinFlip()) {
    ...
    return interpolatedWithSteps.replaceAll(
      /(left|right)/gi,
      (substr) =>
        `<span class="is-negated">${
...
export function fixBinaryInstructions(q: Question) {
  const htmlify = (rule: string) =>
    rule
      .split(', ')
      .map((str) => `<span class="subject">${str}</span>`)
      .join(', ');
  switch (q.type) {
...
export function areGraphsIsomorphic(
  edgeList1: [string, '↔' | '→' | '←', string][],
  edgeList2: [string, '↔' | '→' | '←', string][]
) {
  ...
  function backtrack(index: number) {
    ...
    for (const v of vertices2) {
      if (!used.has(v)) {
        ...
        if (
          graph1[u].in.size === graph2[v].in.size &&
