...
import {
  ...
  QUESTION_TYPE_SETTING_PARAMS,
} from '../constants/settings.constants';
import { jsonCopy } from '../utils/json';
import { b2n } from '../utils/question.utils';

const getNumOfEnabledQuestions = (
  settings: Settings,
  basicQuestionFilter: boolean
) => {
...
const getNumOfEnabledOperators = (settings: Settings) => {
  return Object.values(settings.enabled.binary).reduce((a, c) => a + b2n(c), 0);
};
...
export function canGenerateQuestion(
  ...
  const enoughPremises =
    numOfPremises >= settings.question[questionType].minNumOfPremises;
  if (
    settings.question[questionType].basic ||
    questionType === EnumQuestionType.GraphMatching
  ) {
...
export function areSettingsInvalid(settings: Settings) {
  ...
  const numOfEnabledQuestions =
    numOfEnabledBasicQuestions + getNumOfEnabledQuestions(settings, false);
...
export class QuestionSettings {
  ...
  clampNumOfPremises(numOfPremises: number) {
    return Math.max(
      this.minNumOfPremises,
      Math.min(this.maxNumOfPremises, numOfPremises)
    );
  }
}
